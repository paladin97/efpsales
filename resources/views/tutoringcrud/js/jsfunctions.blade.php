<script>
						...
						<div class="card-body">
							<div class="row">
								<div class="col-sm-12">
									...
									<div class="row">
										<div class="col-sm-12">
											<h4><i class="fad fa-th-list text-bold text-lightblue text-lg"></i> Gestión de Tutorías</h4>
										</div>
									</div>
									<div class="row" style="margin:auto;">
										<div class="col-sm-12">
											<a class="btn bg-lightblue btn-xs managementNote" data-container="body" data-object="`+d.crypt_case_id+`" data-id="`+d.id+`"><i class="fad fa-plus fa-xs"></i> Añadir Nota de tutoría</a>
											<div class="table-responsive">
		...
		columnDefs: [
			...
			},
			{
				render: function ( data, type, row ) {
					return '<a href="tel:'+data+'"><i class="fad text-lightblue fa-lg fa-phone"></i> '+data+'</a>' ;
				}, targets: [6]
			},
			{
				render: function ( data, type, row ) {
					return '<a href="mailto:'+data+'"><i class="fad text-lightblue fa-lg fa-envelope"></i> '+data+'</a>' ;
				}, targets: [7]
			},
			{
				render: function ( data, type, row ) {
    ...
    // $('#leadsModal').on('show.bs.modal', function (event) {
		...
		$.ajax({
			type:"GET",
			url: "{{route('tutoringcrud.index') }}" +'/' + lead_id +'/edit',
			dataType: 'json',
	...
	$('body').on('click', '.viewContractCalendar', function () {
		...
		$('#miniCalendarHeader').html('Calendario de Tutorías');
