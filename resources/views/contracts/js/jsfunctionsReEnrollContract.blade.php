<script>
    $('body').on('click', '.reEnroll', function () {
        $("#contract_actions").val('re-enroll');
        ...
        $.get("{{route('contractcrud.index') }}" +'/' + contract_id +'/edit', function (data) {
            ...
            $('#samepayer').hide();
            $('#modalHeadingContract').html("Ampliar Contrato / Matricular de Nuevo");
            ...
            $('#postpone_enroll').val(data[0].dt_postpone_enroll);
