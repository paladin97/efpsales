<script>
    ...
    //Aqui debemos cargar el select2 con los datos de Contratos
    @foreach(App\Models\WhatsappTemplate::where('template_type_id',2)->orderBy('name','ASC')->get() as $cData) //Contratos
        var data = {
            id: {{$cData->id}},
            text: '{{$cData->name}}'
        };
    ...
    $('#saveBtnWhatsapp').click(function (e) {
        if (!$("#summernoteWhatsapp").val()) {
            ...
            var urlWhats ='https://wa.me/'+phone+'?text='+unescape($("#summernoteWhatsapp").val());
